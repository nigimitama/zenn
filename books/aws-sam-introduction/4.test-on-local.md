...
ただし、ミスリーディングなエラーメッセージが出ることがあります。例えば
...
しかし実際には認証に問題があるわけではなく、正しいパス / メソッドを指定すればきちんと動作して 200 のステータスコードが返ってきます。

こうした、存在しないリソースへのリクエストに対して 403 エラーを返すのは API Gateway の仕様です（[参考](https://repost.aws/knowledge-center/api-gateway-authentication-token-errors)）。
